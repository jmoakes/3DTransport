 ...
 *  
 *  Modified by Jessica Oakes on 2/24/16 HELLO WORLD
 */
...
/*
* This section advects the particles by solving x(new) = x(old) + v*dt
*/
void Advect(LagrangianPoint *pt, double StartTime, double tstart, double tend, int num_particles, double x1, double x2, double x3) {
    ...
    if(pt->LeftDomain && Int_Extrapolate)
        for(ii = 0; ii < 3; ii++) /* Only for massless particles*/
            pt->X[ii] = pt->X[ii] + pt->V[ii] * (tend - tstart);
    else {
        if(Particle_Radius > TINY) { /* goes to Euler integration for particles with a mass*/
            stoptime = pEuler(pt, tstart, tend, num_particles, StartTime, x1, x2, x3);

        }
        else {
            if(Int_Type == 1) 
                stoptime = RK4(pt, tstart, tend); /* Only for massless particles*/
            else if(Int_Type == 2)
                stoptime = RKF(pt, tstart, tend); /* Only for massless particles*/
            else
                stoptime = Euler(pt, tstart, tend); /* Only for massless particles*/
        }
...
double pEuler(LagrangianPoint *pt, double tstart, double tend, int num_particles, double StartTime, double x1, double x2, double x3) {
    /***
    ...
    double u, v, w, d, bx1[3], bx2[3], abn[3], vt[3], vn[3], distance_to_wall;
    int i, ii, jj, sindex, lbdnodes[3], bdnodes[3], count, HitWall, index;
    ...
    while((tc + TINY) < tend) { /* Integration not complete */
        ...
        /*fprintf(stderr,"%i ", Int_NormalFlow);*/
        if(Int_NormalFlow == 1) {
            ...
            for(ii = 0; ii < 26; ii++) {
                ...
                if(sindex >= 0) {
                	fprintf(stderr, "%d ", sindex);
                	fprintf(stderr, "YOU ARE HERE ");
                	fprintf(stderr, "%d ", Vel_MeshElementFlagArray[sindex]);
        ...
        /*fprintf(stderr, "Int_NormalFlow is %d ", Int_NormalFlow);*/                

        if(Int_NormalFlow == 2) {
        	distance_to_wall = 1*Particle_Radius;
        ...
        if((Data_MeshType == CARTESIAN && TestOutsideDomain(pt->X)) || (Data_MeshType == UNSTRUCTURED && pt->ElementIndex < 0) || (Data_MeshType == UNSTRUCTURED && HitWall == 1)) {
            /* Point left domain */
            /*fprintf(stderr, "%d ", pt->ElementIndex);*/
            pt->LeftDomain = 1;
